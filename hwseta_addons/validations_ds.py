...
def enrolment_status_date(value):
    today = date.today()
    value = datetime.datetime.strptime(value, "%d-%m-%Y %H:%M:%S").date()
    # value = value.date()
    # print(value)
    if value > today:
        return False
...
def start_date(startdate):
    ...
    startdate = datetime.datetime.strptime(startdate, "%d-%m-%Y %H:%M:%S").date()
    if startdate > today:
        return False
...
def end_date(enddate):
    today = date.today()
    enddate = datetime.datetime.strptime(enddate, "%d-%m-%Y %H:%M:%S").date()
    if enddate > today:
        return False
...
def person_name(value):
    ...
    if bool(re.findall(check2, value)) == True:
        ...
        return False
    if bool(re.findall(check1, value)) == True:
        dbg('if bool(re.findall(check1, value)) == True:')
        return False
    else:
        return True
...
def vd_check_popi_act_status_date(startdate):
    today = date.today()
    if type(startdate) == str:
        startdate = datetime.datetime.strptime(startdate, "%Y-%m-%d")
    if startdate > today:
        dbg("startdate > today",startdate > today)
        return False
    else:
        return True
...
# Tested and Working
def check_latitude_degree(latitude_degree_value):
    broken = False
    valid = []
    msg = ""
    if latitude_degree_value == "False":
        latitude_degree_value = ""
    if latitude_degree_value:
        valid = latitude_degree(latitude_degree_value)
    if valid == False:
        broken = True
...
# Tested and Working
def check_latitude_minutes(latitude_minutes_value):
    broken = False
    valid = []
    msg = ""
    if latitude_minutes_value == "False":
        latitude_minutes_value = ""
    if latitude_minutes_value:
        valid = latitude_minutes(latitude_minutes_value)
    if valid == False:
        broken = True
...
# Tested and Working
def check_latitude_seconds(latitude_seconds_value):
    broken = False
    valid = []
    msg = ""
    if latitude_seconds_value == "False":
        latitude_seconds_value = ""
    if latitude_seconds_value:
        valid = latitude_seconds(latitude_seconds_value)
    if valid == False:
        broken = True
...
# Tested and Working
def check_longitude_degree(longitude_degree_value):
    broken = False
    valid = []
    msg = ""
    if longitude_degree_value == "False":
        longitude_degree_value = ""
    if longitude_degree_value:
        valid = longitude_degree(longitude_degree_value)
    if valid == False:
        broken = True
...
# Tested and Working
def check_longitude_minutes(longitude_minutes_value):
    broken = False
    valid = []
    msg = ""
    if longitude_minutes_value == "False":
        longitude_minutes_value = ""
    if longitude_minutes_value:
        valid = longitude_minutes(longitude_minutes_value)
    if valid == False:
        broken = True
...
# Tested and Working
def check_longitude_seconds(longitude_seconds_value):
    broken = False
    valid = []
    msg = ""
    if longitude_seconds_value == "False":
        longitude_seconds_value = ""
    if longitude_seconds_value:
        valid = longitude_seconds(longitude_seconds_value)
    if valid == False:
        broken = True
...
def check_provider_end_date(provider_end_date):
    ...
    if provider_end_date == "False":
        ...
        provider_end_date = ""
    try:
        (datetime.datetime.strptime(provider_end_date, "%d-%m-%Y %H:%M:%S").date())
...
# Tested and Working
def check_site_no(site_no_value):
    broken = False
    valid = []
    msg = ""
    if site_no_value == "False":
        msg = "Site No may not be left blank"
        broken = True
        site_no_value = ""
    if site_no_value:
        valid = site_no(site_no_value)
    if valid == False:
        broken = True
...
def check_sic_code(sic_code):
    broken = False
    valid = []
    msg = ""
    if sic_code:
        valid = SIC_code(sic_code)
    if valid == False:
        broken = True
        msg = "SIC Code should only contain ABCDEFGHIJKLMNOPQRTSUVWXYZ1234567890@#&+() /\:._,'`\n"
    return broken, msg
...
def check_employer_postal_address_1(employer_postal_address_1):
    ...
    if employer_postal_address_1 == "False":
        msg = "Postal address_1 field may not be left blank."
        broken = True
...
def check_employer_postal_address_2(employer_postal_address_2):
    ...
    if employer_postal_address_2 == "False":
        msg = "Postal address 2 field may not be left blank."
        broken = True
...
def check_employer_postal_address_code(employer_postal_address_code):
    ...
    if employer_postal_address_code == "False":
        msg = "postal address code field may not be left blank."
    ...
    if valid == False:
        broken = True
        msg = "Postal address code should only contain a valid address code."
    return broken, msg
...
def check_employer_physical_address_1(employer_physical_address_1):
    ...
    if employer_physical_address_1 == "False":
        msg = "Physical address 1 Field may not be left blank."
        broken = True
...
def check_employer_physical_address_2(employer_physical_address_2):
    ...
    if employer_physical_address_2 == "False":
        msg = "Physical address 2 Field may not be left blank."
        broken = True
...
def check_employer_physical_address_code(employer_physical_address_code):
    ...
    if employer_physical_address_code == "False":
        msg = "Physical address code  Field may not be left blank."
        broken = True
    ...
    if valid == False:
        broken = True
        msg = (
            "Physical address code should only contain a valid address code."
        )
    return broken, msg
...
def check_latitude_degree(latitude_degree_value):
    ...
    if valid == False:
        broken = True
        msg = "may only contain whole numbers and may not be greater than -22 and may not have a value less than -35\n"
    return broken, msg
...
def check_latitude_minutes(latitude_minutes_value):
    ...
    if valid == False:
        broken = True
        msg = "may not be greater than 59\n"
    return broken, msg
...
def check_latitude_seconds(latitude_seconds_value):
    ...
    if valid == False:
        broken = True
        msg += "may only contain 1234567890 and must have a length of exactly 6 (nn.nnn) and may not be greater than 59.999.\n"
    return broken, msg
...
def check_longitude_degree(longitude_degree_value):
    ...
    if valid == False:
        broken = True
        msg = "may only contain whole numbers and may not be greater than 33 and may not have a value less than 16\n"
    return broken, msg
...
# Tested and Working
def check_employer_registration_number(employer_registration_number_value):
    broken = False
    valid = []
    msg = ""
    if employer_registration_number_value:
        valid = employer_registration_number(employer_registration_number_value)
    if valid == False:
        broken = True
        msg = "should only contain ABCDEFGHIJKLMNOPQRTSUVWXYZ1234567890@#&+() /\:._-"
    return broken, msg
...
def check_employer_phone_number(employer_phone_number):
    ...
    if valid == False:
        broken = True
        msg = "should ony contain 1234567890"
    return broken, msg
...
def check_employer_fax_number(employer_fax_number):
    ...
    if valid == False:
        broken = True
        msg = "should only contain 1234567890"
    return broken, msg
...
def check_employer_contact_name(employer_contact_name):
    ...
    if valid == False:
        broken = True
        msg = "should ony contain a valid contact name."
    return broken, msg
...
def check_employer_contact_email_address(employer_contact_email_address):
    ...
    if valid == False:
        broken = True
        msg = "should contain a valid email."
    return broken, msg
...
def check_person_postal_address_1(person_postal_address_1):
    ...
    if valid == False:
        broken = True
        msg = "person_postal_address_1 should only contain a valid address \n"
    return broken, msg
...
def check_person_postal_address_2(person_postal_address_2):
    ...
    if valid == False:
        broken = True
        msg = "person_postal_address_2 should only contain a valid address \n"
    return broken, msg
...
def check_person_postal_address_3(person_postal_address_3):
    ...
    if valid == False:
        broken = True
        msg = "person_postal_address_3 should only contain a valid address \n"
    return broken, msg
...
def check_person_postal_address_code(person_postal_address_code):
    ...
    if valid == False:
        broken = True
        msg = "person_postal_address_code should only contain a valid address code\n"
    return broken, msg
...
def check_person_home_address_1(person_home_address_1):
    ...
    if valid == False:
        broken = True
        msg = "person_home_address_1 should only contain a valid address\n"
    return broken, msg
...
def check_person_home_address_2(person_home_address_2):
    ...
    if valid == False:
        broken = True
        msg = "person_home_address_2 should only contain a valid address\n"
    return broken, msg
...
def check_person_home_address_3(person_home_address_3):
    ...
    if valid == False:
        broken = True
        msg = "person_home_address_3 should only contain a valid address\n"
    return broken, msg
...
def check_person_address_code(person_address_code):
    ...
    if valid == False:
        broken = True
        msg = "person_address_code should only contain a valid address code\n"
    return broken, msg
...
def check_person_cell_phone_number(person_cell_phone_number):
    ...
    if valid == False:
        broken = True
        msg = "person_cell_phone_number should ony contain 1234567890 ()/-\n"
    return broken, msg
...
def check_person_fax_number(person_fax_number):
    ...
    if valid == False:
        broken = True
        msg = "person_fax_number should ony contain 1234567890 ()/-\n"
    return broken, msg
...
def check_person_last_name(person_last_name):
    ...
    if valid == False:
        broken = True
        msg = "person_last_name should contain a valid name\n"
    return broken, msg
...
def check_person_first_name(person_first_name):
    ...
    if valid == False:
        broken = True
        msg = "person_first_name should contain a valid name\n"
    return broken, msg
...
def check_person_middle_name(person_middle_name):
    ...
    if valid == False:
        broken = True
        msg = "person_middle_name should contain a valid name\n"
    return broken, msg
...
# Tested and Working
def check_popi_act_status_date(popi_act_status_date):
    broken = False
    valid = []
    msg = ""
    dbg(popi_act_status_date)

    try:

        if popi_act_status_date == "False":
            popi_act_status_date = ""
        if popi_act_status_date:
            valid = vd_check_popi_act_status_date(popi_act_status_date)
        if valid == False:
            broken = True
...
def check_person_previous_alternate_id_type_id(person_previous_alternate_id_type_id_value):
    ...
    # if person_previous_alternate_id_type_id == "False":
...
def check_last_school_year(last_school_year_value):
    # removing this half baked check... how tf do we check for the YYYY format on a date? this would never pass
    #     rather get the year out when reporting...
    return False, ""
    # broken = False
    # valid = []
    # msg = ""
